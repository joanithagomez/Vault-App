...
Vault.createHash = function(key, message, entropy) {
  var CJS =
      typeof CryptoJS !== "undefined" ? CryptoJS : require('./crypto-js-3.1.2.js'),
    bytes = (entropy || 256) / 8;

  return CJS.PBKDF2(key, message, {
    keySize: Math.ceil(bytes / 4),
    iterations: 8
  }).toString();
};
...
Vault.prototype.generate = function(service) {
  ...
  while (result.length < this._length) {
    ...
    while (same && i--)
      same =
        same && result.charAt(result.length + i - this._repeat) === previous;
    if (same) charset = this.subtract([previous], charset.slice());
...
Vault.Stream = function(phrase, service, entropy) {
  ...
  var hash = Vault.createHash(phrase, service + Vault.UUID, 2 * entropy),
    bits = Vault.map(hash.split(""), Vault.toBits)
      .join("")
      .split("");
...
if (typeof module === "object") module.exports = Vault;
